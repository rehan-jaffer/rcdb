...
==== PubChem
* Will check for all entries that have a 'cid' (pubchem ID) present and pull the relevant JSON data into the database where possible. This needs to be updated to pull more relevant pubchem data, as at present it's not really providing any useful extra information, better understanding of pubchem API is required.
==== Sitemap
* Running sitemap:generate will create an XML sitemap for each drug entry and for the relevant listing page, to aid with google's indexing
==== Google Trends
