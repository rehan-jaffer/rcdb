...
=== Requirements
* Postgres >= 9.3 - due to use of hstore, upgrading to 9.4 is in the pipeline to be able to use jsonb over json type for performance reasons (largely due to indexing pubchem data)
...
* Capybara-webkit used for headless integration testing, note that this introduces a libqt5 dependency. In order to avoid this, comment out capybara-webkit in the Gemfile and replace it with selenium, and remove the following line in spec/rails_helper.rb: 
